...
//串口发送缓存区 	
__align(8) uint8_t USART3_TX_BUF[USART_MAX_SEND_LEN]; 	//发送缓冲,最大USART_MAX_SEND_LEN字节 
uint8_t USART3_RX_BUF[USART_MAX_RECV_LEN]; 				    //接收缓冲,最大USART_MAX_RECV_LEN个字节.
...
void GpsDataRead(void)
{
		...
		if( RX_len)
		{
			...
			GPS_Analysis(&gpsx,(uint8_t*)USART3_TX_BUF);//分析字符串，数据存入GPS结构体
