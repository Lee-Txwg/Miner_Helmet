...
void ESP8266_SendData(unsigned char *data, unsigned short len)
{
    ...
    {
        // ESP8266_SendCmd 返回 0，表示成功收到了 '>'
        DEBUG_LOG("Received '>' prompt (SendCmd returned 0). Sending data...\r\n");
        delay_ms(2);
        Usart_SendString(USART2, data, len); // �����豸������������
    }
    else // 如果 ESP8266_SendCmd 返回非 0，表示没收到 '>' 或出错了
    {
        // *** 加这一行日志 ***
        DEBUG_LOG("!!! Error: Failed to get '>' prompt after AT+CIPSEND. Check ESP8266 status! \r\n");
        // 这里先不发送数据，因为没收到 '>'
    }
}
