...
void OneNet_Publish(const char *topic, const char *msg)
{
    ...
    if (MQTT_PacketPublish(MQTT_PUBLISH_ID, topic, msg, strlen(msg), MQTT_QOS_LEVEL0, 0, 1, &mqttPacket) == 0)
    {
        ...
        DEBUG_LOG("Attempting to delete MQTT buffer...\r\n"); // <--- 加日志点1
        MQTT_DeleteBuffer(&mqttPacket);                       // ɾ��
        DEBUG_LOG("MQTT buffer deleted.\r\n");                // <--- 加日志点2
    }
    else
    {
        DEBUG_LOG("Error: Failed to create MQTT Publish packet.\r\n");
        // 5. ***确认点***: 如果 MQTT_PacketPublish 返回失败，是否有可能它内部已经分配了部分内存但没释放？
        //    （通常库函数设计时会避免这种情况，失败时应自动清理）。
        //    在这种情况下，不调用 MQTT_DeleteBuffer 是正确的，但要确保库函数行为符合预期。
        //    可以加日志确认进入了这个分支。
        DEBUG_LOG("Packet creation failed, buffer should not need deletion.\r\n"); // <--- 加日志点3
    }
}
