...
// For ESP8266_ConnectServer
#define MAX_SERVER_RETRIES 3   // maximum reconnect attempts
#define SERVER_RETRY_DELAY 300 // milliseconds between retries
...
int main(void)
{
    ...
    TIM2_Int_Init(4999, 7199); // T = 0.5s
    TIM3_Int_Init(2499, 7199); // T = 0.25
    TIM1_PWM_Init();

    /*--> [2.4] System Ready Indication */
    BEEP = 1;//NPN transistor: Conducts at high level.
    delay_ms(250);
    BEEP = 0;
...
void Comm_Handler(unsigned short *tick)
{
    ...
    char ns_hemi = gpsx.nshemi; // 'N' or 'S'
	  size_t buffer_size; // Variable to store the size
    ...
    if (10 <= ++(*tick))
    {
        ...
        sprintf(PUB_BUF,
				...
				/*--> [2.1.1] Get and print the PUB_BUF size */
        buffer_size = strlen(PUB_BUF);
        // Use %u for unsigned int, common in embedded systems if %zu isn't supported
        DEBUG_LOG("PUB_BUF size: %u bytes\r\n", (unsigned int)buffer_size); // Print the size! 
...
/**
 * @brief  Connects the ESP8266 to the OneNet MQTT server.
 * @note   Calls OneNet_DevLink() up to MAX_SERVER_RETRIES times with SERVER_RETRY_DELAY between attempts.
 *         If all attempts fail, the ESP8266 module is re-initialized via ESP8266_Init().
 */
void ESP8266_ConnectServer(void)
{
    uint8_t retry = 0;

    for (; retry < MAX_SERVER_RETRIES; retry++)
    {
        if (!OneNet_DevLink())
        {
            DEBUG_LOG("ESP8266_ConnectServer succeeded on attempt #%u\r\n", retry + 1);
            return;
        }
        DEBUG_LOG("ESP8266_ConnectServer failed #%u, retrying in %ums\r\n",
                  retry + 1, SERVER_RETRY_DELAY);
        delay_ms(SERVER_RETRY_DELAY);
    }

    // All retries exhausted—re-initialize the ESP8266 module
    DEBUG_LOG("Exceeded %u retries, re-initializing ESP8266", retry);
    ESP8266_Init();
}
