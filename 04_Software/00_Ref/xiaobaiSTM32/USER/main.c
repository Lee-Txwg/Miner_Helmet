...
const char *devSubTopic[] = {"helmet/cmd"};
const char devPubTopic[] = "helmet/status";
...
//			DEBUG_LOG("alarmFlag = %d\r\n", alarmFlag);
		...
		if(++timeCount >= 200)	// 5000ms / 25 = 200 发送间隔5000ms
		{
			...
			DEBUG_LOG("发布数据 ----- OneNet_Publish");
			sprintf(PUB_BUF,"{\"temperature\":%d.%d,\"humidity\":%d.%d,\"smoke\":%d,\"location\":{\"lat\":39.9042,\"lng\":116.4074},\"light\":\"%s\",\"danger\":%s}",
				temperatureH, temperatureL,          // 温度使用已有变量
				humidityH, humidityL,               // 湿度使用已有变量
				0,                                  // smoke假数据设为0
				Led_Status?"on":"off",              // light用已有的Led_Status转换
				alarmFlag?"true":"false");          // danger用已有的alarmFlag转换
			OneNet_Publish(devPubTopic, PUB_BUF);
			DEBUG_LOG("==================================================================================");
			timeCount = 0;
			ESP8266_Clear();
